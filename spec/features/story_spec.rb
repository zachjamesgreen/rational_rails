...
RSpec.describe 'Story features' do
  ...
  it 'should create a new story' do
    ...
    expect(page).to have_current_path("/authors/#{@author2.id}/stories/new")
    fill_in('Name', with: 'SSTTOORRYY')
    click_on('Submit')
    expect(page).to have_current_path("/authors/#{@author2.id}/stories")
    expect(page).to have_content('SSTTOORRYY')
  end

  it 'should update a story' do
    visit("/stories/#{@story2.id}")
    expect(page).to have_link('Update')
    click_on('Update')
    expect(page).to have_current_path("/stories/#{@story2.id}/edit")
    fill_in('Name', with: 'I know this')
    click_on('Submit')
    expect(page).to have_current_path("/stories/#{@story2.id}")
  ...
  it 'should have link to alphabetize stories for author' do
    ...
    visit("/authors/#{author.id}/stories")
    expect(page).to have_link('Sort By Name')
    click_link('Sort By Name')
    expect(page).to have_current_path("/authors/#{author.id}/stories?sort_by_name=true")
  ...
  it 'should have link to update story from story index page and parent story index' do
    visit('/stories')
    expect(page).to have_link('Update', href: "/stories/#{@story1.id}/edit")
    visit("/authors/#{@author1.id}/stories")
    expect(page).to have_link('Update', href: "/stories/#{@story1.id}/edit")
  end

  it 'should delete story' do
    visit("/stories/#{@story3.id}")
    expect(page).to have_link('Delete', href: "/stories/#{@story3.id}")
    click_on('Delete')
    expect(page).to have_current_path('/stories')
  ...
  it 'should delete story from index page' do
    visit('/stories')
    expect(page).to have_content(@story1.name)
    expect(page).to have_link('Delete', href: "/stories/#{@story1.id}")
    expect(page).to have_content(@story4.name)
    expect(page).to have_link('Delete', href: "/stories/#{@story4.id}")
